...
const SubstitutionCard: React.FC<SubstitutionCardProps> = ({ substitution, availableTeachers, onSubstituteChange, isEditing }) => {
  return (
    <div className="bg-white rounded-lg shadow-md border border-slate-200 p-6 transition-all">
      ...
      <div className="bg-slate-50 rounded-md p-4 mt-4 border border-slate-200">
        ...
        {isEditing ? (
          ...
          >
            {!availableTeachers.some(t => t.id === substitution.substituteTeacherId) && (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold text-emerald-700">{substitution.substituteTeacherName}</p>
            </div>
          </div>
        )}
